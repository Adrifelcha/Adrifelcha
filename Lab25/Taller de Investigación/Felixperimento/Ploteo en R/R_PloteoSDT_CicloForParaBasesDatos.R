...
#Guardamos el contenido de nuestros archivos
Condicion_FA <- read.csv("FA_forfor.csv", header=FALSE)
Condicion_Mixta <- read.csv("Mixto_forfor.csv", header=FALSE)

#Para la Condición 1
#El for pasa por cada elemento de la lista
for(x in 1:23){
  h_rate<-Condicion_FA[x,1] #La x va a tomar valores del 1 al 23, el 1 es porque es la primera columna
  fa_rate<-Condicion_FA[x,2] #El 2 es porque toma los valores de la segunda columna
  ...
  #plots
  plot(soporte,d_ruido,type='l',lwd=2, xlab="Eje X", ylab="EjeY")
...
} #Esto es importante porque cierra el For
#-------------------------------------------------------------------------
#Hacemos lo mismo para la Condición 2 a partir del For
for(x in 1:23){
  h_rate<-Condicion_Mixta[x,1] #Lo único que cambia es que la matriz es la que se llama Condicion2
  fa_rate<-Condicion_Mixta[x,2] 
  k<-qnorm(1-fa_rate,0,1)
  d<-qnorm(h_rate,0,1)-qnorm(fa_rate,0,1)
  c<-k-(d/2)                    
  beta<-dnorm(k,d,1)/dnorm(k,0,1)
