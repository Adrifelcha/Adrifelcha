...
La Tabla~\ref{Tabla_t-HitsyFA} presenta el promedio de las $d'$ computadas por cada nivel de dificultad en cada uno de los experimentos realizados, la diferencia en estas y un indicador sobre su significancia estadística. Como se puede apreciar, la diferencia entre las medias de $d'$ estimadas es significativa en ambos experimentos.\\
...
%Mientras que en la literatura clásica en TDS las tasas registradas de Hits y Falsas Alarmas son interpretadas como reflejo directo del área de las distribuciones de Señal y Ruido que caen por encima del criterio de elección, \parencite{Wickens, Gescheider, Stainslaw1999}, bajo el marco del modelamiento bayesiano, el total registrado de Hits y Falsas Alarmas ($H_ij$ y $Fa_ij$) se toma como una instancia de un conteo de casos -o bien, el "número de éxitos"- extraidos de un conjunto definido de observaciones con una probabilidad oculta. Es decir, los Hits y Falsas Alarmas son extraídos de acuerdo a una distribución binomial con parámetros $p= \theta^H_{ij} o \theta^F_{ij}$ y $n= s o n$.\\
...
Como ya se mencionó, la cualidad esencial del modelo desarrollado es que asume que los parámetros $D_{j}$ y $C_{j}$ computados por cada sujeto $i$ son extraídos de distribuciones normales que describen la discriminabilidad ($d'$) y el sesgo ($c$) asociado a cada clase de estímulos $j$. Dichas distribuciones están definidas por los parámetros $\mu^D_{j}$, $\mu^C_{j}$ y $\sigma^D_{j}$, $\sigma^C_{j}$. Es decir:\\
...
Un segundo plato ($j condiciones$) señala que

\item \textbf{Parámetro Delta}\\

Finalmente, se incluye un parámetro Delta que va a computar de manera determinista la diferencia entre los valores estimados para las medias de $d'$ de cada nivel de dificultad, ($\mu^D_{A}$ y $\mu^D_{B}$). Es decir:\\
