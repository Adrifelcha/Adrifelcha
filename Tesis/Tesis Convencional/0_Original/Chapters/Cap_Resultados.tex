...
Cuando las mismas pruebas T para muestras independientes presentadas en la Tabla~\ref{Tabla_t-HitsyFA} se realizan sin la transformación arcoseno de los datos, las diferencias entre las Tasas de Falsas Alarmas no resultan significativas en ningún experimento ($t=1.6536$, $p=0.0533>0.05$ para el Experimento 1 y $t=1.6577$, $p=0.0534>0.05$, para el Experimento 2).\\

Pese a que la transformación arcoseno se realiza de manera justificada, como una forma de prevenir el efecto de suelo y techo que pudiera afectar la evaluación de diferencias entre datos que se encuentren en la cercanía de los límites superior e inferior, la discrepancia entre las conclusiones sugeridas por las pruebas t realizadas con o sin dicha transformación puede llegar a generar dudas acerca de su confiabilidad.\\

El desarrollo de un modelo bayesiano para evaluar las diferencias entre los Hits y las Falsas Alarmas registradas por cada clase de estímulo, presenta como ventaja principal que no se requiere de manipular los datos en forma alguna, en tanto que la estimación paramétrica se hace a partir de inferencias probabilísticas que definen los valores de cada parámetro que, en conjunto, podrían dar cuenta de los datos registrados con mayor probabilidad. Para ello, se desarrolló un modelo bayesiano (identificado en el presente trabajo como Modelo Tau), que incorpora un par de parámetros Tau ($\tau$) que computan las diferencias entre las estimaciones realizadas por cada clase de estímulos ($j$) acerca de las probabilidades ocultas tras la emisión de los Hits y Falsas Alarmas obtenidos por cada participante ($i$).\\ 
...
%\decoRule
\caption[Modelo Tau: Modelo Bayesiano para evaluar las diferencias entre las tasas de Hits y Falsas Alarmas obtenidas por cada clase de estímulos]{Modelo bayesiano desarrollado para evaluar las diferencias entre }
\label{fig:Mod_Tau}
\end{figure}

En las Figuras~\ref{fig:Tau_Hits} y \ref{fig:Tau_FA}
...
Finalmente, en la Figura~\ref{fig:Tau}

\begin{figure}[th]
\centering
\includegraphics[width=0.7\textwidth]{Figures/MTau_DensidadTau_E1} \& \includegraphics[width=0.7\textwidth]{Figures/MTau_DensidadTau_E1}\\
%\decoRule
\caption[Modelo Delta: Densidad de probabilidad posterior para el valor del parámetro Delta]{Modelo jerárquico bayesiano desarrollado a partir del supuesto de que los valores de $d'$ y sesgo $c$ computados por cada participante en cada clase provienen de su propia distribución normal. El modelo incorpora un parámetro ($\delta$) que estima la diferencia entre las medias de $d'$ estimadas por cada condición. El modelo tiene priors no informativas.}
\label{fig:Tau}
\end{figure}
