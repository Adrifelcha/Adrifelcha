...
%\decoRule
\caption[Variabilidad en la señal y en el ruido]{Extensión del ejemplo sobre la variabilidad en los puntajes obtenidos en una prueba clínica por pacientes con depresión, donde además de la distribución asociada a la señal (en azul), se añade una distribución para representar el rango de puntajes observados en personas que no tienen depresión (el ruido; en negro). El sobrelape entre ambas distribuciones representa la incertidumbre contenida en la tarea de detección: existe un rango de puntajes que se puede esperar de una persona con, o sin depresión.}
\label{fig:Noise}
\end{figure}
...
%\decoRule
\caption[El sobrelape Ruido-señal como reflejo de la incertidumbre contenida en tareas de detección]{La distancia entre las distribuciones de ruido y señal determina el sobrelape entre las mismas y por tanto, la incertidumbre contenida en la tarea de detección. En el panel a) las distribuciones aparecen muy separadas, el sobrelape es pequeño y se asume poca incertidumbre.  En el panel b) las distribuciones están más cerca, comparten más evidencia en el área de sobrelape y hay una mayor incertidumbre.}
\label{fig:Overlap}
\end{figure}
...
%\decoRule
\caption[Matriz de contingencia con los posibles Resultados en una Tarea de Detección]{Los cuatro posibles resultados a encontrar en una tarea de detección de señales, de acuerdo con la correspondencia entre los juicios emitidos por los organismos y el estado del mundo.}
...
%\decoRule
\caption[Ejemplo de Matriz de Pagos]{EJEMPLO: Matriz de pagos con los costos y ganancias comprometidos en la tarea de detectar un depredador.}
\label{fig:Mat_Pagos}
\end{figure}
...
%\decoRule
\caption[Estimación del criterio con base en las Falsas Alarmas]{Estimación de la localización del criterio sobre el eje de decisión con base en las tasas de Falsas Alarmas y Rechazos Correctos.}
...
%\decoRule
\caption[Estimación del sesgo con base en el criterio]{Se ilustra el cómputo de los parámetros de sesgo $\beta$ (figuras izquierdas) y $C$ (derechas), en casos donde el sesgo del sistema es liberal (el criterio se ubica en niveles bajos de evidencia; panel superior) o conservador (el criterio se sitúa sobre niveles altos de evidencia; panel inferior).}
