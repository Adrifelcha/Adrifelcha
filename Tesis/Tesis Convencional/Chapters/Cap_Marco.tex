...
\item Todos los estímulos comienzan con una cierta proporción de rasgos marcados como "familiares"\\
\begin{center}
$p(A,nuevo) = p(new)$\\
$p(B,nuevo) = p(new)$\\
\end{center}

\item Las clases de estímulos A y B difieren en \\
\begin{center}
$n(A) > n(B)$\\
\end{center}

\item Una vez que son presentados en la fase de estudio, se marcan como "conocidos" los rasgos que componen cada estímulo:\\
\begin{center}
$\alpha(A) = \frac{n(A)}{N}$\\
$\alpha(B) = \frac{n(B)}{N}$\\
\end{center}
...
\item Al presentarse una segunda vez en la fase experimental, los estímulos viejos tienen una mayor proporción de rasgos marcados que los estímulos nuevos:\\
\begin{center}
...
De tal forma que de acuerdo con la Teoría de Atención/Verosimilitud, en la fase de reconocimiento se asume que los participantes responden con base en la cantidad de rasgos "marcados" muestreados de cada estímulo a evaluar. El número de rasgos marcados extraídos en cada ensayo ($x$) se distribuye de acuerdo a una distribución binomial con probabilidad $p(i,j)$ (donde $j$ es el tipo de ensayo: nuevos o viejos) y un total de observaciones $n(i)$ (el número de rasgos atendidos en función a la clase del estímulo).\\
...
En otras palabras, en términos de la Teoría de Atención/Verosimilitud el elemento clave para explicar las discrepancias en el desempeño de los participantes entre ambas clases de estímulos es la atención elicitada por cada uno de estos y el número de rasgos muestreados en consecuencia ($n(i)$). Es decir, que aunque los estímulos A y B contienen el mismo número de rasgos marcados en su primera presentación ($p(A,nuevo) = p(B,nuevo)$), el número de elementos muestreados para su marcaje es mayor en la condición A ($n(A) > n(B)$) y por tanto, se extrae una mayor cantidad de rasgos marcados en A que en B desde su primera presentación ($p(x|p(A,nuevo),n(A)) < p(x|p(B,nuevo),n(B))$).\\

Sin embargo, pese al conjunto de experimentos desarrollados para probar la solidez de la Teoría de Atención/Verosimilitud mediante la manipulación de distintas variables experimentales que se asume tienen incidencia sobre los distintos parámetros propuestos por el modelos (Por ejemplo: restringiendo el tiempo de estudio para modificar $n(i)$ en la fase de estudio o el tiempo de respuesta para modificar $n(i)$ en la fase experimental, etc.) y evaluar la precisión con que el modelo propuesto predice y explica los datos encontrados, \parencite{Glanzer1993, Kim1993, Glanzer1991}, la Teoría de Atención/Verosimilitud ha recibido muchas críticas $CITAS$ \CITAS
