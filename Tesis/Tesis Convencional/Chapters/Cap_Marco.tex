...
Una segunda aproximación, (strength)\\
...
Una tercera aproximación implica asumir que las clases de estímulos empleadas en estos estudios tienen un efecto sobre los procesos superiores involucrados en las tareas de reconocimiento. En otras palabras, este segundo conjunto de explicaciones asume que cada clase de estímulos A y B es procesada de manera diferencial por el participante. Por ejemplo, una de las primeras teorías en desarrollarse para dar cuenta del Efecto Espejo en Memoria de Reconocimiento es la Teoría de Atención/Verosimilitud \parencite{Glanzer1993}. Dicha teoría funciona como una teoría de muestreo de rasgos en la que se asume que todos los estímulos a presentar están compuestos por un número fijo de rasgos ($N$), de los cuales, algunos se presentan "marcados" desde su primera aparición como "rasgos familiares" ($p(new$). Esta teoría asume que la diferencia fundamental entre  las clases de estímulos probadas ($i$) es que elicitan distintos gradientes de atención que van a repercutir en el número de rasgos atendidos por los participantes ($n(i)$) de $N$, definiendo una tasa de  "marcaje" propia de cada clase ($\alpha(i)$).\\
...
\begin{numerate}
\item Todos los estímulos están compuestos por una cantidad $N$ de rasgos que pueden estar marcados o no como "conocidos".\\
...
\item A y B difieren en el número de rasgos que los participantes muestrean de sus elementos al interactuar con ellos, debido a que elicitan distintos niveles de atención.\\
\begin{center}
$n(A) > n(B)$\\
\end{center}
\begin{center}
...
\item Al muestrear los rasgos muestreados en la interacción con los estímulos son marcados. De acuerdo con $n(i)$, A y B tienen su propia tasa de marcaje.\\
\begin{center}
$\alpha(A) = \frac{n(A)}{N}$\\
$\alpha(B) = \frac{n(B)}{N}$\\
\end{center}
\begin{center}
donde $n(A) > n(B)$ y por tanto, $\alpha(A) > \alpha(B)$\\
\end{center}

\item En la fase de estudio, los participantes muestrean cierto número de rasgos y marcan aquellos que no lo estén con anterioridad. De manera que en la fase de reconocimiento, los estímulos presentados previamente tienen una mayor proporción de rasgos marcados que los estímulos nuevos:\\
\begin{center}
...
De acuerdo con la Teoría de Atención/Verosimilitud, los participantes registran sus respuestas en la fase de reconocimiento con base en la cantidad de rasgos "marcados" muestreado ($x$). La probabilidad de observar cierto valor de $x$ se describe en relación a una distribución binomial con probabilidad $p(i,j)$ (donde $j$ es el tipo de ensayo: nuevos o viejos), para el total de observaciones $n(i)$ (el número de rasgos atendidos en función a la clase del estímulo). Es decir:\\

\begin{center}
$p(x|p(i,j),n(i))$\\
\end{center}

El modelo asume que los participantes observan una $x$ cantidad de rasgos marcados en cada ensayo y emiten el juicio de reconocimiento que corresponda a dicha evidencia con mayor probabilidad, en función al cómputo de la razón de verosimilitud.\\

De acuerdo con esta teoría, el Efecto Espejo se explica por medio de la siguiente relación:\\

\begin{center}
$p(x|p(A,nuevo),n(A)) < p(x|p(B,nuevo),n(B)) < p(x|p(B,viejo),n(B)) < p(x|p(A,viejo),n(A))$\\
\end{center}

En otras palabras, en la Teoría de Atención/Verosimilitud el elemento clave para explicar las diferencias en el desempeño de los participantes entre A y B es la atención elicitada por cada clase y el número de rasgos muestreados en consecuencia ($n(i)$). Es decir, que aunque los estímulos A y B contienen el mismo número de rasgos marcados en su primera presentación ($p(A,nuevo) = p(B,nuevo)$), el número de elementos muestreados es mayor en la condición A ($n(A) > n(B)$) y por tanto, se extrae una mayor cantidad total de rasgos marcados que en B ($p(x|p(A,nuevo),n(A)) < p(x|p(B,nuevo),n(B))$).\\

Sin embargo, pese al conjunto de experimentos desarrollados para probar la solidez de la Teoría de Atención/Verosimilitud mediante la manipulación de distintas variables experimentales que deberían incidir sobre los distintos parámetros del modelos (Por ejemplo: restringiendo el tiempo de estudio para modificar $n(i)$ en la fase de estudio o el tiempo de respuesta para modificar $n(i)$ en la fase experimental, etc.) y evaluar la precisión con que el modelo predice y explica los datos encontrados, \parencite{Glanzer1993, Kim1993, Glanzer1991}, la Teoría de Atención/Verosimilitud ha recibido muchas críticas  \parencite{Hintzman1994, Murdock1998, DeCarlo2007}.\\
...
Por otro lado, se encuentran los modelos y teorías desarrollados para explicar por qué el Efecto Espejo refleja la no pertinencia de emplear el modelo de detección de Señales al estudio de la Memoria de Reconocimiento.

Como se mencionó previamente, uno de los supuestos que se aceptan por "default" al usar el modelo de detección de señales para describir el desempeño de los participantes en tareas de reconocimiento es que existe tal cosa como un "eje de evidencia" sobre el cual se despliegan distintos valores de "fuerza de memoria" (o "familiaridad"). 

Por ejemplo, dado que el Efecto Espejo ha sido demostrado con consistencia en estudios donde se utilizan palabras muy comúnes y palabras poco comúnes, uno esperaría que si agregáramos al esperimento una tercera condición compuesta por palabras "Raras", estas fueran mucho menos familiares en su primer presentación y fueran más fáciles de recordar en una segunda presentación (tal y como predeciría el Efecto Espejo explicado en términos de la "familiaridad" de los estímulos evaluados). Sin embargo, este no parece ser el caso \parencite{Rao1984, Wixted1992}
