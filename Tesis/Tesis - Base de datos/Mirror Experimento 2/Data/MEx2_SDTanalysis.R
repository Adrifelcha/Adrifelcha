...
rm(list=ls())
#pdf_name<-'SDT-Classical.pdf'
pdf(file="~/Desktop/Felisa/Tesis/Tesis - Base de datos/Mirror Experimento 2/Data/PDFS_Exp2/SDT-Classical.pdf", width=8, height=8)
...
rm(list=ls())
#pdf_name<-'SDT-MirrorEffect_ABBA.pdf'
pdf(file="~/Desktop/Felisa/Tesis/Tesis - Base de datos/Mirror Experimento 2/Data/PDFS_Exp2/SDT-MirrorEffect_ABBA.pdf", width=8, height=8)
...
#  beta_B <-dnorm(k_B,d_B,1)/dnorm(k_B,0,1)
  print(c("k",k[length(k)], 
          "d_A", d_A[length(d_A)], 
          "d_BN",d_BN[length(d_BN)], 
          "d_BS",d_BS[length(d_BS)], 
          "C",c[length(c)],
          archive))
...
#
  ...
  print(c(h_AS6[length(h_AS6)], 
          ...
          fa_BN2[length(fa_BN2)],
          fa_BN1[length(fa_BN1)]))
  ...
  k1_B <- qnorm(1-fa_BN1,0,1)
  k2_B <- qnorm(1-fa_BN2,0,1)
  k3_B <- qnorm(1-fa_BN3,0,1)
  k4_B <- qnorm(1-h_BS4,d_BS,1)
  k5_B <- qnorm(1-h_BS5,d_BS,1)
  k6_B <- qnorm(1-h_BS6,d_BS,1)
  }
  soporte <- seq(-3.5,7,.05)
...
####### Confidence Rating
  ...
  #abline(v=k2_B,col='forestgreen', lty=1, lwd=2)
  #abline(v=k3_B,col='darkgreen', lty=1, lwd=2)
  #abline(v=k4_B,col='black', lty=1, lwd=2)
  #abline(v=k5_B,col='darkgoldenrod3', lty=1, lwd=2)
  #abline(v=k6_B,col='orange', lty=1, lwd=2)
