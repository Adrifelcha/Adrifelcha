...
for(archive in dir()){
  ...
  axis(1,at=a,labels=a)
  axis(2,at=c(1,2,3,4), labels=c('F.Alarm', 'Miss', 'Rejection', 'Hit'), font=2)
}
...
for(archive in dir()){
  ...
  plot(jaime$RTime1,type='o',pch=16, col='purple',ylim=c(0,16),axes=F , ylab='Seconds', xlab='Trial', font.lab=2 )
  axis(1,at=a,labels=a)
...
for(archive in dir()){
  ...
  mtext("Number of External Circles", side = 1, line = 2.5, cex = 1, font = 2)
...
for(archive in dir()){
  ...
  mtext("Hits", side = 2, line = 3, cex = 1, font = 2, las = 0)
  mtext('Hits per color',3,cex=1.5, font=2)
  title(archive, outer = TRUE, line = -2)
  ...
  mtext("Color", side = 1, line = 2.5, cex = 1, font = 2)
  mtext("False Alarms", side = 2, line = 3, cex = 1, font = 2, las = 0)
  mtext('F. Alarms per color',3,cex=1.5, font=2)
  }
