setwd("C:/Users/Adrifelcha/Dropbox/Tesis/Experimentos/Mirror Experimento 1/Data_Ex1a_SESGO")
...
for(archive in dir()){
  ...
  orden_color <- function(tamanos_centrals){
    return(
      data.frame(
        estim=felisa$Estimulo[tamano_centrales==tamanos_centrals],
        label=rep(c(rep('naranja',2),rep('azul',2),rep('verde',2),rep('purpura',2),rep('guinda',2)),8)[order(order(felisa$Estimulo[tamano_centrales==tamanos_centrals]))],
        posicion_glob=which(tamano_centrales==tamanos_centrals)
      )  
    )
  }
  ...
  tipo[(felisa$Estimulo>=1&felisa$Estimulo<=200)] <- '4 AS'
  tipo[(felisa$Estimulo>=401&felisa$Estimulo<=600)] <- '3 BS'
  tipo[(felisa$Estimulo>=201&felisa$Estimulo<=400)] <- '1 AN'
  tipo[(felisa$Estimulo>=601&felisa$Estimulo<=800)] <- '2 BN'
