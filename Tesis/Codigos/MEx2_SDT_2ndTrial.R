...
#print(c(samples[length(samples)]))


# Now the values for the monitored parameters are in the "samples" object, 
# ready for inspection.
for(d in archive){
d_A <- samples$BUGSoutput$sims.list$d_A[archive]
d_B <- samples$BUGSoutput$sims.list$d_B[archive]
}

for(c in archive){
  c_A <- samples$BUGSoutput$sims.list$c_A[archive]
  c_B <- samples$BUGSoutput$sims.list$c_B[archive]
}

for(h in archive){
  h_A <- samples$BUGSoutput$sims.list$thetah_A[archive]
  h_B <- samples$BUGSoutput$sims.list$thetah_B[archive]
}

for(f in archive){
  f_A <- samples$BUGSoutput$sims.list$thetaf_A[archive]
  f_B <- samples$BUGSoutput$sims.list$thetaf_B[archive]
}
...
plot(density(d_A), lwd=2, col="red", main="", ylab="", xlab="", 
     xlim=c(-2,6), axes=F)
axis(1)
