...
lines(x, g, lwd=2, col="blue")   #Sobreponemos una línea que represente Función 2    (el valor asignado por la función g a cada valor contenido en x)
legend(0.75, 1.6, c("f(x)", "g(x)"), lty=c(1,1), lwd=2, col=c("red", "blue"))     #Añadimos un recuadro/leyenda que señale a qué corresponde cada línea. Separamos los elementos en un arreglo.
...
fit <- optimize(g, c(0, 1), maximum=TRUE)   #Usamos la función optimize para obtener el punto máximo (maximum=TRUE) la función g, en el intervalo 0 y 1
...
f1 <- function(x) {6*x*(1-x)}       #Especificamos la función 
integral <- integrate(f1, lower = 0, upper = 1)      #Integramos la función anteriormente definida en el intervalo 0  a 1
