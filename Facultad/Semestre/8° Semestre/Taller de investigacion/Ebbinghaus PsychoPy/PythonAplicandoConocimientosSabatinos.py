...
#Abre los archivos para guardar datos
mybaseline=open(options["Nombre"]+ time.strftime("%Y-%m-%d")+ time.strftime("%H.%M.%S")+ "Baseline.csv", 'w')

mytestline=open(options["Nombre"]+ time.strftime("%Y-%m-%d")+ time.strftime("%H.%M.%S")+ "TestLine.csv", 'w')
...
#Indicaciones de los encabezados del primer archivo de datos
...
#Primer loop de la Fase de Entrenamiento
mywindow.update()
for i in range (10):
...
#Se especifica las respuestas validas ante ada ensayo
	userSel=" "
	validkeys=['s', 'n']
	while userSel !='s' and userSel !='n':
		...
		for k in keys:
			if k in validkeys:
				...
				core.quit()
		core.wait(0.1)
#Se programa la clasificacion Correcto e incorrecto
...
#Se programan el resto de clasificaciones
...
#Se presentan los resultados al participante
mywindow.setColor(color=[255,255,255], colorSpace="rgb255")
mywindow.update()

mywindow.setColor(color=[255,255,255], colorSpace="rgb255")
mywindow.update()
txtResults=visual.TextStim(mywindow,text="Resultados: "+"\n"+"\n"+"Respuesatas Correctas: "+str(rightcounterb)+"\n"+"Respuestas Incorrectas: "+str(wrongcounterb),color=[0,0,0], colorSpace="rgb255")
txtResults.draw()
mywindow.update()
core.wait(2)

#Instrucciones Fase de Prueba
mywindow.setColor(color=[255,255,255], colorSpace="rgb255")
mywindow.update()
mywindow.setColor(color=[255,255,255], colorSpace="rgb255")
mywindow.update()
...
#Se pone titulo a cada columna del segundo archivo de datos
...
#Empieza el loop de la Fase de Prueba
mywindow.update()
for i in range (10):
	choice=central[random.randint(0,len(central)-1)]
	if choice==1:
	...
	mywindow.update()
#Se programan las respuestas aceptadas por ensayo
	userSel=" "
	validkeys=['s', 'n']
	while userSel !='s' and userSel !='n':
		keys=event.getKeys()
